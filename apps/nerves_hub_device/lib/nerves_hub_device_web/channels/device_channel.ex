defmodule NervesHubDeviceWeb.DeviceChannel do
  ...
  def join("firmware:" <> fw_uuid, _payload, socket) do
    with {:ok, certificate} <- get_certificate(socket),
         ...
         deployments <- Devices.get_eligible_deployments(device) do
      %{update_available: update_available} = join_reply = resolve_update(device.org, deployments)
      Phoenix.PubSub.subscribe(NervesHubWeb.PubSub, "device:#{device.id}")
      send(self(), {:after_join, device, update_available})
  ...
  defp resolve_update(org, [%Deployments.Deployment{} = deployment | _]) do
    with {:ok, firmware} <- Firmwares.get_firmware(org, deployment.firmware_id),
      ...
      %{update_available: false, firmware_url: url}
    else
  ...
  def handle_info(
        {:after_join,
         %Device{id: device_id, last_known_firmware_id: firmware_id, org: %Org{id: org_id}},
         update_available},
        socket
      ) do
    {:ok, _} =
